...
# Experience Replay特定配置
ER_BUFFER_SIZE = 20
ER_REPLAY_BATCH_SIZE = 8
ER_REPLAY_WEIGHT = 0.5
...
# ==================== Replay Buffer ====================
class ReplayBuffer:
    def __init__(self, max_size=ER_BUFFER_SIZE):
...
# 为每个任务创建一个buffer
buffers = [ReplayBuffer(max_size=ER_BUFFER_SIZE) for _ in range(NUM_TASKS)]

# 性能追踪矩阵
results_matrix = np.zeros((NUM_TASKS, NUM_TASKS))

# ==================== 顺序连续学习 + Experience Replay ====================
print("\n" + "="*70)
print("方法: Experience Replay连续学习")
print(f"配置: Buffer大小={ER_BUFFER_SIZE}, Replay批次={ER_REPLAY_BATCH_SIZE}, ")
print(f"      Replay权重={ER_REPLAY_WEIGHT}")
print("="*70)
...
for i in range(NUM_TASKS):
        ...
        if i > 0:
            for old_task_idx in range(i):
                if not buffers[old_task_idx].is_empty():
                    old_obs, old_target, old_task_id = buffers[old_task_idx].sample(ER_REPLAY_BATCH_SIZE)
                    old_output = model(old_obs, old_task_id)
                    replay_loss = criterion(
                        old_output.view(-1, MAX_OUTPUT),
                        old_target.view(-1)
                    )
                    loss += ER_REPLAY_WEIGHT * replay_loss
    ...
    print(f"  → 任务 {current_task} 的数据已加入Replay Buffer")
