...
- **学习方法优化思路**：把模型权重参数整体视为一个参数空间，假设每一轮训练数据学习对参数空间中参数点的作用是线性的，参数点被柔性限制在几个方向构成的子空间内（类似罚函数机制），在补空间内不移动或移动很少，可以认为子空间特征向量方向对旧任务是关键的，新的学习鼓励改变补空间方向的参数点移动，惩罚子空间方向的参数点移动，为了限制子空间方向移动，在新的训练数据中加入与新数据（在倒数第二个特征层的权重参数的余弦）相似度较大的旧数据一起训练，起到用旧数据限制旧任务主导参数方向的作用。这个连续学习方法已有先例，称为相似性加权交错学习（SWIL）
